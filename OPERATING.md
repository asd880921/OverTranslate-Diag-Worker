...
┃
┃  ID                                                   回報代碼
┃  20260101093000-3f2504e0-4f89-41d3-9a0c-0305e82c3301  A3F-7K2
┃
┃  版本          系統               大小
...
訊息裡只有 ID、代碼、版本、系統、大小——就是 key metadata 的那幾項，跟儲存空間裡存的完全一樣，不含 IP。**包本身不會送過去**：那是別人螢幕上的文字，它留在儲存空間裡，要拿得自己跑取檔腳本。

ID 那格顯示 `None` 表示這包來自還不會送識別碼的舊版 App——那不是壞掉，是這則訊息在告訴你對方的版本有多舊。
...
看 ID。相同就是同一個安裝，Discord 訊息上就看得到，不用把包抓下來。

它是 App 第一次啟動時自己產生的（時間戳 + 隨機 GUID），存在自己的設定檔裡，之後每次上傳都附上。它不從機器或人身上取任何東西，所以反查不到誰；重灌會換一個新的；使用者在自己包裡的 `appsettings.redacted.json` 就讀得到它。它只回答「這兩份回報是不是同一個安裝」這一個問題。

顯示 `None` 表示對方的 App 舊到還不會送這個值。那時候只剩包裡的旁證：log 裡的顯示器快照（裝置名稱、精確解析度、DPI），以及設定的形狀（快捷鍵、配色、擷取目標）。足以形成「大概是同一台機器」的判斷，不足以當根據。

反過來說，ID 不同**不代表**不是同一個人：換電腦、重灌、砍掉設定檔都會換號。它證明得了「是同一個」，證明不了「不是同一個」。
